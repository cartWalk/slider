...
(function(fn){
    ...
    $.fn.slider = function(callBack){
      ...
      var nbr = $sliderItem.length;
      var sliderItemMarginRight = parseInt($sliderItem.css('margin-right'));
      var windowWidth = $('.slider-window').width();
      var OFFSET = $sliderItem.width() + sliderItemMarginRight; // 每走一个的距离
      var sliderListWidth = $sliderItem.width() * nbr + (nbr - 1) * sliderItemMarginRight;
      var MAXMARGINLEFT = sliderListWidth  - windowWidth - sliderItemMarginRight;
      var aviableOffset = 0;
      if(MAXMARGINLEFT > 0){
          var offset = 0; // 当前的偏移量
          var index = 1;
          ...
          $next.click(function(){
              aviableOffset = sliderListWidth - windowWidth + offset;
              console.log(2)
              if(Math.abs(offset)  >= MAXMARGINLEFT){ 
                  return;
              }

              if(aviableOffset < OFFSET){
                  offset = offset - aviableOffset;
                  $sliderList.animate({
                  'margin-left':offset + 'px'
                  }, 800);
              }else{
        ...
        arguments.length ? callBack() : (function(){ return;})();
    }
})
